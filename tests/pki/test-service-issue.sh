...
assert_no_residue() {
  ...
  if compgen -G "$pki/intermediate-ca/.platform-pki-service-issue.*" >/dev/null; then
...
openssl verify -CAfile "$pki/root-ca/certs/root-ca.crt" \
  -untrusted "$pki/intermediate-ca/certs/intermediate-ca.crt" "$cert" >/dev/null || \
...
[[ $(<"$pki/intermediate-ca/serial") == 1001 ]] || fail 'intermediate serial did not advance'
[[ $(wc -l <"$pki/intermediate-ca/index.txt") -eq 1 ]] || fail 'intermediate index did not advance once'
[[ -f $pki/intermediate-ca/newcerts/1000.pem ]] || fail 'intermediate newcert was not published'
assert_no_residue "$pki"

key_hash=$(file_hash "$key")
cert_hash=$(file_hash "$cert")
index_hash=$(file_hash "$pki/intermediate-ca/index.txt")
serial_hash=$(file_hash "$pki/intermediate-ca/serial")
...
[[ $(file_hash "$pki/intermediate-ca/index.txt") == "$index_hash" ]] || fail 'existing-certificate refusal changed index'
[[ $(file_hash "$pki/intermediate-ca/serial") == "$serial_hash" ]] || fail 'existing-certificate refusal changed serial'
...
rm "$pki/services/rotate/certs/tls.crt" "$pki/services/rotate/csr/tls.csr" \
  ...
  "$pki/services/rotate/openssl.cnf"
...
rollback_index_hash=$(file_hash "$rollback_pki/intermediate-ca/index.txt")
rollback_serial_hash=$(file_hash "$rollback_pki/intermediate-ca/serial")
rollback_newcerts=$(find "$rollback_pki/intermediate-ca/newcerts" -maxdepth 1 -type f -printf '%f\n' | sort)
...
[[ $(file_hash "$rollback_pki/intermediate-ca/index.txt") == "$rollback_index_hash" ]] || fail 'signing failure changed index'
[[ $(file_hash "$rollback_pki/intermediate-ca/serial") == "$rollback_serial_hash" ]] || fail 'signing failure changed serial'
[[ $(find "$rollback_pki/intermediate-ca/newcerts" -maxdepth 1 -type f -printf '%f\n' | sort) == "$rollback_newcerts" ]] || fail 'signing failure changed newcerts'
...
for config_case in include database-escape randfile-escape; do
  ...
  config_path="$config_pki/intermediate-ca/openssl.cnf"
  case $config_case in
    include)
  ...
  config_index_hash=$(file_hash "$config_pki/intermediate-ca/index.txt")
  config_serial_hash=$(file_hash "$config_pki/intermediate-ca/serial")
  run_command "$TOOL" failure --namespace "$config_namespace" \
    --intermediate-pass-file "$INT_PASS"
  assert_status 1
  assert_contains "$STDERR" "$expected_config_error"
  [[ $(file_hash "$config_pki/intermediate-ca/index.txt") == "$config_index_hash" ]] || fail "$config_case changed index"
  [[ $(file_hash "$config_pki/intermediate-ca/serial") == "$config_serial_hash" ]] || fail "$config_case changed serial"
  [[ ! -e $config_pki/services/failure ]] || fail "$config_case created service state"
  [[ ! -e $config_pki/root-ca/.platform-pki-root-operation.lock ]] || fail "$config_case acquired root lock"
...
inventory_parent_serial_hash=$(file_hash "$inventory_parent_pki/intermediate-ca/serial")
...
[[ $(file_hash "$inventory_parent_pki/intermediate-ca/serial") == "$inventory_parent_serial_hash" ]] || fail 'inventory-parent symlink changed serial'
[[ ! -e $inventory_parent_pki/root-ca/.platform-pki-root-operation.lock ]] || fail 'inventory-parent symlink acquired root lock'
...
printf '%s\n' abcd >"$serial_success_pki/intermediate-ca/serial"
...
[[ $(<"$serial_success_pki/intermediate-ca/serial") == ABCE ]] || fail 'lowercase serial did not advance canonically'
[[ -f $serial_success_pki/intermediate-ca/newcerts/ABCD.pem ]] || fail 'lowercase serial did not publish canonical newcert name'
...
printf '%s\n' 00ab >"$serial_collision_pki/intermediate-ca/serial"
printf '%s\n' sentinel >"$serial_collision_pki/intermediate-ca/newcerts/AB.pem"
chmod 600 "$serial_collision_pki/intermediate-ca/newcerts/AB.pem"
serial_collision_hash=$(file_hash "$serial_collision_pki/intermediate-ca/newcerts/AB.pem")
...
[[ $(file_hash "$serial_collision_pki/intermediate-ca/newcerts/AB.pem") == "$serial_collision_hash" ]] || fail 'canonical serial collision replaced existing newcert'
...
[[ $(file_hash "$verify_pki/intermediate-ca/index.txt") == "$verify_index_hash" ]] || fail 'verification failure changed index'
[[ $(file_hash "$verify_pki/intermediate-ca/serial") == "$verify_serial_hash" ]] || fail 'verification failure changed serial'
...
for publication_case in failure:1 HUP:129 TERM:143; do
  ...
  publication_index_hash=$(file_hash "$publication_pki/intermediate-ca/index.txt")
  publication_serial_hash=$(file_hash "$publication_pki/intermediate-ca/serial")
  ...
  [[ $(file_hash "$publication_pki/intermediate-ca/index.txt") == "$publication_index_hash" ]] || fail "$case_name publication changed index"
  [[ $(file_hash "$publication_pki/intermediate-ca/serial") == "$publication_serial_hash" ]] || fail "$case_name publication changed serial"
...
[[ -d $recovery_pki/root-ca/.platform-pki-root-operation.lock ]] || fail 'recovery race did not retain root lock'
[[ -d $recovery_pki/intermediate-ca/.platform-pki-intermediate-operation.lock ]] || fail 'recovery race did not retain intermediate lock'
recovery_stage=$(compgen -G "$recovery_pki/intermediate-ca/.platform-pki-service-issue.??????")
...
for unsafe_case in symlink hardlink mode type inventory; do
  ...
  case $unsafe_case in
    ...
    hardlink)
      ...
      ;;
    mode)
      chmod 777 "$unsafe_pki/intermediate-ca/newcerts"
...
EOF
  ...
  unsafe_serial_hash=$(file_hash "$unsafe_pki/intermediate-ca/serial")
  run_command "$TOOL" failure --namespace "$unsafe_namespace" --intermediate-pass-file "$INT_PASS"
  assert_status 1
  assert_contains "$STDERR" "$expected"
  [[ $(file_hash "$unsafe_pki/intermediate-ca/serial") == "$unsafe_serial_hash" ]] || fail "$unsafe_case validation changed CA state"
  [[ ! -e $unsafe_pki/root-ca/.platform-pki-root-operation.lock ]] || fail "$unsafe_case validation acquired root lock"
done
...
run_command env PATH="$EXEC_DIR/owner-bin:$PATH" REAL_STAT="$REAL_STAT" \
  STAT_OWNER_TARGET="$owner_pki/intermediate-ca/index.txt" \
...
[[ ! -e $owner_pki/root-ca/.platform-pki-root-operation.lock ]] || fail 'owner validation acquired root lock'

lock_namespace="$TMP_DIR/lock"
create_ca "$lock_namespace"
lock_pki="$lock_namespace/pki"
mkdir -m 700 "$lock_pki/intermediate-ca/.platform-pki-intermediate-operation.lock"
...
[[ -d $lock_pki/intermediate-ca/.platform-pki-intermediate-operation.lock ]] || fail 'contended lock was removed'
[[ ! -e $lock_pki/root-ca/.platform-pki-root-operation.lock ]] || fail 'root lock was not released after contention'
rmdir "$lock_pki/intermediate-ca/.platform-pki-intermediate-operation.lock"
...
[[ -d $isolation_pki/root-ca/.platform-pki-root-operation.lock ]] || fail 'issuance did not hold root lock'
[[ -d $isolation_pki/intermediate-ca/.platform-pki-intermediate-operation.lock ]] || fail 'issuance did not hold intermediate lock'
