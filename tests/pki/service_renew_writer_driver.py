...
from src.platform_pki.service_issue import renew_managed_service
    ...
    try:
        return renew_managed_service(
            ...
            fault_hook=FaultHook(
                crash_at=environment.get("PLATFORM_PKI_SERVICE_RENEW_CRASH_AT"),
                signal_at=environment.get("PLATFORM_PKI_SERVICE_RENEW_SIGNAL_AT"),
                failure_at=environment.get("PLATFORM_PKI_SERVICE_RENEW_FAILURE_AT"),
                signum=int(environment.get("PLATFORM_PKI_SERVICE_RENEW_SIGNAL", "15")),
            ),
            pause_hook=PauseHook(
                pause_at=environment.get("PLATFORM_PKI_SERVICE_RENEW_PAUSE_AT"),
                marker=environment.get("PLATFORM_PKI_SERVICE_RENEW_PAUSE_MARKER"),
                release=environment.get("PLATFORM_PKI_SERVICE_RENEW_PAUSE_RELEASE"),
            ),
        )
