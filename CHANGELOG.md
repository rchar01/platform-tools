...
- Changed legacy PKI handling to prepare missing private control directories before locking and reject normal CA and service operations with explicit migration guidance. Inventory installation, protected backup, and rollover status/migration remain available, while root creation rejects mixed legacy and generation authority paths before mutation.
