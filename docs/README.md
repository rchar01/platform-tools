...
- [`../bin/platform-pki`](../bin/platform-pki): Unified Python PKI migration interface with shared runtime, filesystem, fork-safe ordered advisory-lock, and bounded Linux durable-publication primitives; guarded existing-destination replacement and operational handlers remain unavailable during migration.
