...
- Added the deterministic, standard-library-only `platform-pki` Python 3.14
  zipapp as the unified migration interface. The foundation build provides
  isolated help, version, command dispatch, and fail-closed unavailable-handler
  behavior; all operational `platform-pki-*` commands remain unchanged on Bash.
