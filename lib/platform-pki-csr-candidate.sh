...
pki_candidate_validate_sources() {
  ...
  [[ ${CANDIDATE_RESPONSE[target]} == "$INVENTORY_TARGET" && ${CANDIDATE_RECORD[target]} == "$INVENTORY_TARGET" && ${CANDIDATE_ARTIFACT[target]} == "$INVENTORY_TARGET" ]] || pki_die 'CSR candidate target does not match inventory'
...
pki_candidate_decision_content() {
  ...
  printf '%s\n' \
    'schema=1' "action=$CANDIDATE_ACTION" "state=$state" "service=$SERVICE" "target=$INVENTORY_TARGET" "request_id=$REQUEST_ID" "operation=${CANDIDATE_RESPONSE[operation]}" \
...
pki_candidate_resume_finalization() {
  ...
  CANDIDATE_FINALIZATION_JOURNAL[active_destination_identity]=${CANDIDATE_FINALIZATION_JOURNAL[active_stage_identity]}; CANDIDATE_FINALIZATION_JOURNAL[phase]=active-published; pki_candidate_write_journal
...
pki_candidate_validate_recorded_outcome() {
  ...
  [[ ${CANDIDATE_DEPLOYMENT[schema]} == 1 && ${CANDIDATE_DEPLOYMENT[request_id]} == "$REQUEST_ID" && ${CANDIDATE_DEPLOYMENT[artifact_request_id]} == "$REQUEST_ID" && ${CANDIDATE_DEPLOYMENT[service]} == "$SERVICE" && ${CANDIDATE_DEPLOYMENT[target]} == "$INVENTORY_TARGET" && ${CANDIDATE_DEPLOYMENT[deployment_principal]} == "$INVENTORY_TARGET" ]] || pki_die 'Recorded deployment evidence identity is invalid'
  ...
  [[ ${CANDIDATE_DEPLOYMENT[local_certificate_sha256]} == "$CERTIFICATE_SHA256" && ${CANDIDATE_DEPLOYMENT[local_key_spki_sha256]} == "$CERTIFICATE_SPKI_SHA256" && ${CANDIDATE_DEPLOYMENT[local_key_certificate_match]} == true && ${CANDIDATE_DEPLOYMENT[validation_boundary_sha256]} == "$VALIDATION_BOUNDARY_SHA256" ]] || pki_die 'Recorded deployment evidence local or validation-boundary binding failed'
  ...
  pki_csr_verify_signature "$outcome/deployers.allowed_signers" "$INVENTORY_TARGET" platform-pki-csr-deployment-v1 "$outcome/deployment.sig" "$outcome/deployment" 'Recorded deployment evidence'
...
pki_candidate_run() {
  ...
  pki_load_inventory_snapshot "$CANDIDATE_WORK_DIR/inventory"; pki_require_service_in_inventory "$SERVICE"; [[ $(pki_inventory_key_custody "$SERVICE") == host-local ]] || pki_die 'CSR candidate command requires key_custody: host-local'
