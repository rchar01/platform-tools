...
require_safe_destination_layout() {
  ...
  for path in \
    ...
    "$PKI_DIR/authorities" "$PKI_DIR/authorities/roots" \
    "$PKI_DIR/authorities/intermediates" "$PKI_DIR/state" \
    "$PKI_DIR/state/generation-reservations" "$PKI_DIR/state/rollover" \
  ...
  path="$PKI_DIR/inventory/services.yml.example"
    if [[ -e $path || -L $path ]]; then
...
for dir in \
  ...
  "$PKI_DIR/state" "$PKI_DIR/state/generation-reservations" "$PKI_DIR/state/rollover" "$PKI_DIR/locks" \
