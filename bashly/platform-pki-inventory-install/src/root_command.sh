...
finish_install() {
  ...
  if [[ ${PRESERVE_PUBLICATION_STATE:-false} == true ]]; then
    ...
    pki_warn "Inventory publication requires recovery; retained artifacts under: $recovery_dir"
  else
...
require_atomic_mv() {
  ...
  for option in --no-copy none-fail; do
    grep -F -- "$option" <<<"$help" >/dev/null || \
      pki_die 'GNU mv with --no-copy and --update=none-fail is required'
  done
}
...
probe_exchange_mv() {
  ...
  [[ ${PLATFORM_PKI_FORCE_RENAME_FALLBACK:-} != 1 ]] || return 0
  first=$(mktemp "$PINNED_INVENTORY_DIR/.platform-pki-exchange-a.XXXXXX") || pki_die 'Cannot stage exchange preflight'
  second=$(mktemp "$PINNED_INVENTORY_DIR/.platform-pki-exchange-b.XXXXXX") || { rm -f "$first"; pki_die 'Cannot stage exchange preflight'; }
  if command mv --exchange --no-copy -T -- "$first" "$second" 2>/dev/null; then EXCHANGE_SUPPORTED=true; fi
  rm -f -- "$first" "$second"
}
...
publish_existing_destination() {
  ...
  if [[ $EXCHANGE_SUPPORTED != true ]]; then
    # Cooperative locks exclude platform-tools peers. Same-UID hostile processes
    # remain inside the documented trust boundary and are not defeated here.
    recheck_destination
    [[ $(stat -c '%d:%i' "$PUBLICATION_GUARD") == "$expected" ]] || pki_die 'Inventory guard changed before fallback publication'
    mv -f -T -- "$STAGE" "$DESTINATION" || preserve_publication 'Cannot atomically replace active inventory'
    [[ $(stat -c '%d:%i' "$DESTINATION") == "$staged_identity" && $(stat -c '%d:%i' "$PUBLICATION_GUARD") == "$expected" ]] || \
      preserve_publication 'Fallback inventory publication identity check failed'
    STAGE=''
    remove_owned_path "$PUBLICATION_GUARD" "$PUBLICATION_GUARD_IDENTITY" || preserve_publication 'Cannot remove fallback inventory guard'
    PUBLICATION_GUARD=''
    remove_owned_path "$PUBLICATION_GUARD_BASE" "$PUBLICATION_GUARD_BASE_IDENTITY" || preserve_publication 'Cannot remove fallback inventory guard base'
