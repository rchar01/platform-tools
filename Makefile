...
## Run protected offline workspace initialization tests
test-pki-offline-workspace:
	python3 -m pytest tests/pki/test_offline_workspace.py
...
## Run non-public Python host-local CSR issue/migration writer tests
test-pki-csr-issue-writer:
	python3 -m pytest --durations=20 tests/pki/test_csr_issue_writer.py
...
## Run public Python managed and host-local renewal writer tests
test-pki-service-renew-writer:
	...
	python3 -m pytest --durations=20 tests/pki/test_service_renew_writer.py
	python3 -m pytest -n "$$PKI_PYTEST_WORKERS" --dist load --durations=20 \
		tests/pki/test_csr_renew_writer.py

## Run managed service transaction recovery tests
...
## Run authenticated terminal CSR outcome export tests
test-pki-csr-outcome:
	python3 -m pytest tests/pki/test_csr_outcome.py
...
## Run host-key-pinned direct PKI exchange client tests
test-pki-direct-exchange:
	python3 -m pytest tests/pki/test_direct_exchange.py

## Run exact GitLab PKI package transport tests
test-pki-gitlab-package:
	python3 -m pytest tests/pki/test_gitlab_package.py
